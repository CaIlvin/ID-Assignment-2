...
$(document).ready(function () {
      ...
      $.ajax(settings).done(function (response) {
        ...
        for (var i = 0; i < response.length && i < limit; i++) {
          ...
          */
          ...
          <td><a href='#' class='delete' data-id='${response[i]._id}'>Del</a></td><td><a href='#update-contact-container' class='update' data-id='${response[i]._id}' data-name='${response[i].name}' data-score='${response[i].score}' data-time='${response[i].time}'>Update</a></td></tr>`;
    ...
    function updateForm(id, contactName, contactEmail, contactMsg) {
      ...
      var settings = {
        "async": true,
        "crossDomain": true,
        "url": `https://scoreboard-0ca7.restdb.io/rest/player/${id}`,//update based on the ID
        "method": "PUT",
        "headers": {
          "content-type": "application/json",
          "x-apikey": APIKEY,
